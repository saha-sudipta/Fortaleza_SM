...
shp_CT@data <- left_join(shp_CT@data, shp_bairro@data[,-1], by=c("CD_GEOCODB"="cod_ibge"))
```

Project CT shapefile into the same projection as point and bairro data
...
Merge points and shapes

```{r}
tot_homs_bairros <- over(homs_01_17, shp_bairro)
tot_homs_bairros <- tot_homs_bairros %>% group_by(cod_ibge) %>% summarise(count = n())
df <- left_join(shp_bairro@data, tot_homs_bairros, by="cod_ibge")
tot_homs_bairros <- shp_bairro
tot_homs_bairros@data <- df
rm(df)
```
